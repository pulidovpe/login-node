	...
	function(req, email, password, done) { // callback with email and password from our form
		...
		User.findOne({ 'local.email' :  email }, function(err, user) {
			...
			if (!user.validPassword(password))
				return done(null, false, req.flash('loginMessage', 'Error. Clave incorrecta.')); // create the loginMessage and save it to session as flashdata
			...
			if (!req.user) {
				...
				User.findOne({ 'twitter.id' : profile.id }, function(err, user) {
					...
					if (user) {
						...
						// set the user's local credentials
						console.log(`Token twitter: ${newUser.twitter.token}`);
			...
			if (!req.user) {
				...
				User.findOne({ 'google.id' : profile.id }, function(err, user) {
					...
					if (user) {
						...
						// set the user's local credentials
						console.log(`Token google: ${refreshToken.access_token}`);
						newUser.local.email    = profile.emails[0].value;
						newUser.local.password = "sinclave";
						// save the user
						newUser.save(function(err) {
